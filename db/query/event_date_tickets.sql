...
WHERE o_i.option_user_id = sqlc.arg(option_user_id) AND e_d_t.id = sqlc.arg(ticket_id) AND e_d.id = sqlc.arg(event_date_id) AND CAST(CONCAT(e_d_t.start_date, ' ', e_d_t.start_time) AS timestamp) < NOW() AND CAST(CONCAT(e_d_t.end_date, ' ', e_d_t.end_time) AS timestamp) > NOW() AND e_d.status = 'on_sale' AND o_i.is_complete = true AND o_i.is_active = true AND u.is_active = true AND (o_i_s.status = 'list' OR o_i_s.status = 'staged') AND  e_d.is_active = true AND e_d_t.is_active = true;
