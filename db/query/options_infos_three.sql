...
WHERE oi.is_complete = true AND oi.is_active = true AND u.is_active = true AND u.is_deleted = false AND ois.status != 'unlist' AND ois.status != 'snooze' AND (l.city = $1 OR l.street ILIKE $2 OR CAST(earth_distance(
